...
//到增加方法页面
function showAddMethod(entityId) {
    layer.open({
        type: 2,
        title: '新增',
        shade: 0.8,
        ...
        content: getRootPath()+'/entitiesCtl/goAddMethod/'+entityId,
        btn: ['提交', '关闭'], //只是为了演示
        yes: function(index,layero){
            ...
            saveMethod(index,getRootPath()+'/searchMethodCtl/doAdd',data,entityId);
...
function deleteById(methodId,obj,entityId) {
    layer.confirm('您确定要删除？', {
        ...
        function (index,layero) {
            ...
            $.ajax({
                type: "post",
                url: getRootPath()+"/searchMethodCtl/deleteById/"+methodId,
                async:false,
                //data: '{"id":"'+methodId+'"}',
                contentType: "application/json; charset=utf-8",
                //dataType: "json",
...
function showUpdate(id,obj,enitityId) {
    layer.open({
        ...
        content: getRootPath()+'/searchMethodCtl/goUpdate?id='+id+"&entityId="+enitityId,
        btn: ['更新', '取消'], //只是为了演示
        yes: function(index,layero){
            var form = layer.getChildFrame('form',index);//父页面获取子frame中DOM元素
            var data = $(form).serializeObject();//把表单数据转化成json数据
            doUpdate(index,getRootPath()+'/searchMethodCtl/doUpdate',JSON.stringify(data),enitityId);
